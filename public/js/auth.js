var Auth = {
  ...
  bind: function() {
    var self = this;
    var loginBtn = document.getElementById('btn-login'); if (loginBtn) loginBtn.addEventListener('click', function() { UI.openModal('modal-login'); });
    var regBtn = document.getElementById('btn-register'); if (regBtn) regBtn.addEventListener('click', function() { UI.openModal('modal-register'); });
    var switchToReg = document.getElementById('switch-to-register'); if (switchToReg) switchToReg.addEventListener('click', function(e) { e.preventDefault(); UI.closeModal('modal-login'); UI.openModal('modal-register'); });
    var switchToLog = document.getElementById('switch-to-login'); if (switchToLog) switchToLog.addEventListener('click', function(e) { e.preventDefault(); UI.closeModal('modal-register'); UI.openModal('modal-login'); });
    var loginForm = document.getElementById('login-form'); if (loginForm) loginForm.addEventListener('submit', function(e) { e.preventDefault(); self.login(); });
    var regForm = document.getElementById('register-form'); if (regForm) regForm.addEventListener('submit', function(e) { e.preventDefault(); self.register(); });
    var logoutBtn = document.getElementById('btn-logout'); if (logoutBtn) logoutBtn.addEventListener('click', function() { self.logout(); });
    var menuBtn = document.getElementById('user-menu-btn');
    if (menuBtn) menuBtn.addEventListener('click', function(e) { e.stopPropagation(); var dd = document.getElementById('user-dropdown'); if (dd) dd.classList.toggle('open'); });
    document.addEventListener('click', function(e) { if (!e.target.closest('.umenu')) { var dd = document.getElementById('user-dropdown'); if (dd) dd.classList.remove('open'); } });
    var profileBtn = document.getElementById('btn-profile'); if (profileBtn) profileBtn.addEventListener('click', function() { document.getElementById('user-dropdown').classList.remove('open'); self.showProfile(); });
    var myReportsBtn = document.getElementById('btn-my-reports'); if (myReportsBtn) myReportsBtn.addEventListener('click', function() { document.getElementById('user-dropdown').classList.remove('open'); self.showMyReports(); });
    var adminBtn = document.getElementById('btn-admin'); if (adminBtn) adminBtn.addEventListener('click', function() { document.getElementById('user-dropdown').classList.remove('open'); self.showAdmin(); });
  },
  ...
  login: async function() {
    ...
    try {
      var { error } = await App.supabase.auth.signInWithPassword({ email: email, password: password });
      if (error) {
        var msg = 'Erreur de connexion';
        if (error.message.includes('Invalid login')) msg = 'Email ou mot de passe incorrect';
        else if (error.message.includes('Email not confirmed')) msg = 'Confirmez votre email';
  ...
  register: async function() {
    ...
    var errEl = document.getElementById('register-error'); var btn = document.getElementById('btn-register-submit');
    ...
    try {
      var { data, error } = await App.supabase.auth.signUp({ email: email, password: password, options: { data: { username: username, commune: commune } } });
      if (error) {
        var msg = 'Erreur'; if (error.message.includes('already registered')) msg = 'Email déjà utilisé';
        errEl.textContent = msg; errEl.style.display = 'block';
      } else {
        if (commune && data.user) await App.supabase.from('profiles').update({ commune: commune }).eq('id', data.user.id);
        UI.closeModal('modal-register');
        if (data.session) UI.toast('Bienvenue ' + username + ' !', 'success');
  ...
  logout: async function() { await App.supabase.auth.signOut(); App.currentUser = null; App.currentProfile = null; this.showOut(); UI.toast('Déconnecté', 'info'); },

  loadProfile: async function() {
    if (!App.currentUser) return;
    try { var { data } = await App.supabase.from('profiles').select('*').eq('id', App.currentUser.id).single(); if (data) App.currentProfile = data; } catch (e) {}
  },
  ...
  showIn: function() {
    ...
    var adminSection = document.getElementById('admin-section'); if (adminSection) adminSection.style.display = (p && p.role === 'admin') ? 'block' : 'none';
  },
  ...
  showProfile: function() {
    ...
    var html = '<div style="padding:16px"><div style="text-align:center;margin-bottom:16px">' +
      ...
      '<div class="sc"><div class="sc__v" style="font-size:1.2rem;color:var(--green)">' + (p ? p.reputation || 0 : 0) + '</div><div class="sc__l">Reputation</div></div>' +
      '<div class="sc"><div class="sc__v" style="font-size:1.2rem;color:var(--purple)">' + (p ? p.role || 'citizen' : 'citizen') + '</div><div class="sc__l">Role</div></div></div>' +
      '<div class="field"><label>Commune</label><div style="display:flex;gap:6px"><select class="inp" id="profile-commune" style="flex:1"><option value="">Choisir...</option>' +
      ...
      '<optgroup label="Les Saintes"><option>Terre-de-Haut</option><option>Terre-de-Bas</option></optgroup><optgroup label="La Desirade"><option>La Desirade</option></optgroup></select>' +
    ...
    var cs = document.getElementById('profile-commune'), commune = (p && p.commune) || '';
    if (cs && commune) { for (var i = 0; i < cs.options.length; i++) { if (cs.options[i].value === commune || cs.options[i].text === commune) { cs.selectedIndex = i; break; } } }
  ...
  changePassword: async function() {
    ...
    if (error) UI.toast('Erreur', 'error'); else { UI.toast('Mot de passe modifié', 'success'); document.getElementById('profile-new-password').value = ''; document.getElementById('profile-confirm-password').value = ''; }
  },
  ...
  changeCommune: async function() {
    ...
    if (error) UI.toast('Erreur', 'error'); else { App.currentProfile.commune = commune; UI.toast('Commune mise à jour', 'success'); }
  },
  ...
  showMyReports: function() {
    ...
    if (myReports.length === 0) { container.innerHTML = '<div class="empty" style="padding:40px"><span><i class="fas fa-inbox fa-2x"></i></span><h3>Aucun signalement</h3></div>'; }
    else {
  ...
  deleteOwnReport: async function(reportId) {
    ...
    try {
      ...
      if (error) { UI.toast('Erreur', 'error'); return; }
  ...
  updateStatus: async function(reportId, status) {
    ...
    if (error) UI.toast('Erreur', 'error'); else UI.toast('Statut mis à jour', 'success');
  },

  updateResponse: async function(reportId, response) {
    var { error } = await App.supabase.from('reports').update({ admin_response: response, updated_at: new Date().toISOString() }).eq('id', reportId);
    if (error) UI.toast('Erreur', 'error'); else UI.toast('Réponse sauvegardée', 'success');
  },
