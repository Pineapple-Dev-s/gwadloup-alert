var Reports = {
  loadAll: async function() {
    try {
      ...
      for (var i = 0; i < App.reports.length; i++) {
        MapManager.addReport(App.reports[i]);
      }
      this.renderList();
      this.updateStats();
    } catch (e) {
  ...
  renderList: function() {
    ...
    for (var i = 0; i < App.reports.length; i++) {
      ...
      if (r.images && r.images.length > 0) {
        imgHtml = '<img class="card__img" src="' + r.images[0] + '" alt="" loading="lazy">';
      } else {
        imgHtml = '<div class="card__ph"><i class="fas ' + fa + '"></i></div>';
      }
  ...
  updateStats: function() {
    ...
    var pending = 0, inProgress = 0, resolved = 0;
    ...
    el = document.getElementById('stats-resolved'); if (el) el.textContent = resolved;
  ...
  renderCharts: function() {
    ...
    var catSorted = Object.keys(catCounts).sort(function(a, b) { return catCounts[b] - catCounts[a]; }).slice(0, 8);
    var catMax = catSorted.length > 0 ? catCounts[catSorted[0]] : 1;

    var catEl = document.getElementById('chart-categories');
    if (catEl) {
      var html = '';
      for (var i = 0; i < catSorted.length; i++) {
        var k = catSorted[i];
        var label = (App.categories[k] || App.categories.other).label;
        var pct = Math.round((catCounts[k] / catMax) * 100);
        html += '<div class="bar"><span class="bar__label">' + label + '</span>' +
          '<div class="bar__track"><div class="bar__fill" style="width:' + pct + '%"><span class="bar__val">' + catCounts[k] + '</span></div></div></div>';
      }
    ...
    var comSorted = Object.keys(comCounts).sort(function(a, b) { return comCounts[b] - comCounts[a]; }).slice(0, 8);
    var comMax = comSorted.length > 0 ? comCounts[comSorted[0]] : 1;

    var comEl = document.getElementById('chart-communes');
    if (comEl) {
      var html = '';
      for (var i = 0; i < comSorted.length; i++) {
        var k = comSorted[i];
        var pct = Math.round((comCounts[k] / comMax) * 100);
        html += '<div class="bar"><span class="bar__label">' + k + '</span>' +
          '<div class="bar__track"><div class="bar__fill" style="width:' + pct + '%"><span class="bar__val">' + comCounts[k] + '</span></div></div></div>';
      }
      comEl.innerHTML = html || '<p style="color:var(--text3);font-size:.8rem">Pas de données</p>';
    }
  },
  ...
  renderLeaderboard: async function() {
    ...
    try {
      ...
      if (!data || data.length === 0) {
        el.innerHTML = '<p style="color:var(--text3);font-size:.8rem;text-align:center;padding:16px">Pas encore de contributeurs</p>';
        return;
      }
      ...
      el.innerHTML = '<p style="color:var(--text3);font-size:.8rem">Erreur</p>';
    }
  },
  ...
  openDetail: async function(id) {
    ...
    if (App.currentUser) {
      try {
        var { data: vote } = await App.supabase.from('votes').select('id').eq('report_id', id).eq('user_id', App.currentUser.id).single();
    ...
    var html = galHtml +
      ...
      '<span class="badge" style="background:' + priority.color + '22;color:' + priority.color + '">' + priority.label + '</span>' +
      '</div>' +
    ...
    // Status change (everyone can do it)
    if (App.currentUser) {
      html += '<div style="background:var(--bg3);border-radius:var(--r);padding:12px;margin-bottom:16px">' +
        ...
        '<button class="btn btn--outline' + (report.status === 'pending' ? ' btn--primary' : '') + '" onclick="Reports.changeStatus(\'' + id + '\',\'pending\')">En attente</button>' +
        '<button class="btn btn--outline' + (report.status === 'acknowledged' ? ' btn--primary' : '') + '" onclick="Reports.changeStatus(\'' + id + '\',\'acknowledged\')">Vu</button>' +
        '<button class="btn btn--outline' + (report.status === 'in_progress' ? ' btn--primary' : '') + '" onclick="Reports.changeStatus(\'' + id + '\',\'in_progress\')">En cours</button>' +
        '<button class="btn btn--outline' + (report.status === 'resolved' ? ' btn--primary' : '') + '" onclick="Reports.changeStatus(\'' + id + '\',\'resolved\')">Résolu</button>' +
        '</div></div>';
    }
    ...
    if (App.currentUser) {
      html += '<div class="cmtform">' +
        '<textarea id="comment-input-' + id + '" placeholder="Ajouter un commentaire..." rows="2"></textarea>' +
    ...
    try {
      ...
      for (var i = 0; i < comments.length; i++) {
        ...
        html += '<div class="cmt">' +
          ...
          '<div class="cmt__body">' +
          '<div class="cmt__head"><span class="cmt__author">' + App.esc(name) + '</span>' +
      ...
      el.innerHTML = '<p style="color:var(--text3);font-size:.78rem">Erreur</p>';
    }
  },
  ...
  changeStatus: async function(reportId, newStatus) {
    ...
    if (error) {
      ...
      if (report) {
        ...
        if (newStatus === 'resolved') report.resolved_at = updates.resolved_at;
      }
      this.renderList();
      this.updateStats();
  ...
  submitReport: async function() {
    ...
    if (!MapManager.isInGuadeloupe(lat, lng)) {
      UI.toast('L\'emplacement doit être en Guadeloupe', 'error');
      return;
    }
    ...
    try {
      ...
      if (modData.flagged) {
        if (modData.reformulated && modData.cleaned) {
          ...
          UI.toast('Contenu reformulé pour respecter la charte', 'info');
        } else {
  ...
  handleUpdate: function(report) {
    for (var i = 0; i < App.reports.length; i++) {
      if (App.reports[i].id === report.id) {
        App.reports[i] = report;
        break;
      }
    }
