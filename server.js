...
const analytics = {
  ...
  track: function(data) {
    ...
    this.liveVisitors.set(data.sessionId, {
      lastSeen: Date.now(), page: data.page, device: data.device,
      browser: data.browser, country: data.country, referrer: data.referrer
    });
    if (this.liveVisitors.size > this.peakConcurrent) this.peakConcurrent = this.liveVisitors.size;
  },
  ...
  aggregateDaily: async function() {
    ...
    try {
      ...
      await supabaseAdmin.from('analytics_daily').upsert({
        date: today, pageviews: events.length, visitors: sessions.size,
        new_visitors: newSessions.size, mobile: mobile, desktop: desktop, tablet: tablet,
        top_pages: topN(pages,20), top_referrers: topN(referrers,15),
        top_browsers: topN(browsers,10), top_countries: topN(countries,15),
        hourly: hourly, updated_at: new Date().toISOString()
      }, { onConflict: 'date' });
    } catch(e) {}
  },
  ...
  getStats: async function(days) {
    ...
    try {
      var now = new Date();
      var startDate = new Date(now); startDate.setDate(startDate.getDate() - days);
      var startStr = startDate.toISOString().split('T')[0];
      var todayStr = now.toISOString().split('T')[0];
      var dailyResult = await supabaseAdmin.from('analytics_daily').select('*').gte('date', startStr).order('date', { ascending: true });
      ...
      for (var i = 0; i < daily.length; i++) {
        var d = daily[i];
        totalPV += d.pageviews || 0; totalV += d.visitors || 0;
        totalMobile += d.mobile || 0; totalDesktop += d.desktop || 0; totalTablet += d.tablet || 0;
        merge(allPages, d.top_pages || {}); merge(allReferrers, d.top_referrers || {});
        merge(allBrowsers, d.top_browsers || {}); merge(allCountries, d.top_countries || {});
        merge(allHourly, d.hourly || {});
      }
      var sortObj = function(obj, limit) {
        return Object.entries(obj).sort(function(a,b){return b[1]-a[1]}).slice(0,limit)
          .map(function(e){return{name:e[0],count:e[1]}});
      };
      return {
        period: days + ' jours', periodPageviews: totalPV, periodVisitors: totalV,
        todayPageviews: (daily.find(function(d){return d.date===todayStr})||{}).pageviews||0,
        currentConcurrent: this.liveVisitors.size, peakConcurrent: this.peakConcurrent,
        daily: daily.map(function(d){return{date:d.date,pageviews:d.pageviews,visitors:d.visitors}}),
        topPages: sortObj(allPages,20), topReferrers: sortObj(allReferrers,15),
        browsers: sortObj(allBrowsers,10), countries: sortObj(allCountries,15),
        hourly: allHourly,
...
function parseTrackingData(req) {
  ...
  var referrer = null;
  if (ref) { try { var rh = new URL(ref).hostname.replace('www.',''); if (rh && rh !== req.hostname && rh !== 'localhost') referrer = rh; } catch(e) {} }
  var device = /iPad|tablet|Tab/i.test(ua) ? 'tablet' : /Mobile|Android|iPhone/i.test(ua) ? 'mobile' : 'desktop';
  var browser = /Edg/i.test(ua)?'Edge':/OPR|Opera/i.test(ua)?'Opera':/Firefox/i.test(ua)?'Firefox':/Safari/i.test(ua)&&!/Chrome/i.test(ua)?'Safari':/Chrome/i.test(ua)?'Chrome':'Autre';
  var country = 'Inconnu';
  if (lang) { var p = lang.split(',')[0].split('-'); var r = p.length>1?p[1].toUpperCase():'';
...
function hashStr(s) { var h=0; for(var i=0;i<s.length;i++){h=((h<<5)-h)+s.charCodeAt(i);h=h&h} return 'v'+Math.abs(h).toString(36); }
...
function containsBadWords(text) {
  ...
  var lower = text.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[_\-\.]/g,' ').replace(/\s+/g,' ');
  var found = [], severity = 'none';
  for (var i = 0; i < ALWAYS_FLAG.length; i++) {
    var nw = ALWAYS_FLAG[i].normalize('NFD').replace(/[\u0300-\u036f]/g,'');
    if (lower.includes(nw)) { found.push(ALWAYS_FLAG[i]); if (SOFT_INSULTS.indexOf(ALWAYS_FLAG[i])===-1) severity='hard'; else if(severity!=='hard') severity='soft'; }
  }
  for (var i = 0; i < BAD_WORDS.length; i++) {
    if (found.indexOf(BAD_WORDS[i])!==-1) continue;
    var nw = BAD_WORDS[i].normalize('NFD').replace(/[\u0300-\u036f]/g,'');
    if (new RegExp('(?:^|\\W)'+nw.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')+'(?:$|\\W)','i').test(' '+lower+' ')) {
      found.push(BAD_WORDS[i]); if (SOFT_INSULTS.indexOf(BAD_WORDS[i])===-1) severity='hard'; else if(severity!=='hard') severity='soft';
    }
  }
...
app.post('/api/analytics/track', limit(180, 60000), function(req, res) {
  ...
  var td = parseTrackingData(req); td.page = page; td.isNew = true;
  analytics.track(td);
  res.json({ ok: true });
});
...
app.get('/api/wiki-static', limit(30, 60000), function(req, res) {
  ...
  res.json(fs.readdirSync(dir).filter(function(f) { return f.endsWith('.md'); }).map(function(f) {
...
app.post('/api/moderate', limit(60, 60000), async function(req, res) {
  ...
  if (!key) {
    ...
    for (var i = 0; i < check.words.length; i++) { var w = check.words[i]; var r = new RegExp(w.replace(/[.*+?^${}()|[\]\\]/g,'\\$&'),'gi'); ct = ct.replace(r,'...'); cd = cd.replace(r,'...'); }
  ...
  try {
    ...
    var resp = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      ...
      ],temperature:0.3,max_tokens:context==='wiki'?2000:600}),
      signal:ctrl.signal
    });
    clearTimeout(to);
...
app.post('/api/report-anonymous', limit(10, 60000), async function(req, res) {
  ...
  if (check.found) {
    for (var i = 0; i < check.words.length; i++) {
      ...
      title = title.replace(r,'...'); description = description.replace(r,'...');
    }
  }
  try {
    if (!supabaseAdmin) return res.status(500).json({ error: 'Database unavailable' });
    var result = await supabaseAdmin.from('reports').insert({
      category: body.category, title: title, description: description,
      latitude: body.latitude, longitude: body.longitude,
      address: body.address || null, commune: body.commune || null,
      images: [], priority: body.priority || 'medium',
      status: 'pending', upvotes: 0, user_id: null
    }).select().single();
...
// ═══════════════ NOTIFICATION EMAIL (via Supabase Edge Function or direct) ═══════════════
app.post('/api/notify', limit(20, 60000), async function(req, res) {
  // This endpoint is called internally when a comment or admin response is added
  // It checks if the report owner has an email and sends a notification
  var reportId = req.body.reportId;
  var type = req.body.type; // 'comment' or 'admin_response'
  ...
  try {
    ...
    if (!report.data || !report.data.user_id) return res.json({ ok: false }); // anonymous report
    
    // Get owner email from auth
    var profile = await supabaseAdmin.from('profiles').select('username').eq('id', report.data.user_id).single();
    
    // We can't send email directly from here without an email service
    // But we can track that a notification should be sent
    // The actual email sending would be done via Supabase Edge Functions or Resend API
...
analytics.init();
app.listen(PORT, function() { console.log('Gwadloup Alert v16 — port ' + PORT + ' — ' + GROQ_KEYS.length + ' Groq — Anonymous ON — AI Category ON'); });
